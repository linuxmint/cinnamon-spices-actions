...
```sh
sudo apt-get update
sudo apt-get install -y imagemagick zenity libnotify-bin img2pdf poppler-utils file findutils coreutils
```
...
| **Vector Tracing**    | [potrace](https://potrace.sourceforge.net/)                                          | `sudo apt-get install -y potrace`            |
...
**Note on SVG Support**: Currently, SVG support is limited to **monochrome (black & white)**. The conversion uses `potrace`, which traces bitmaps into vector shapes but does not support color

to install all of them at once, run:

```sh
sudo apt-get update
sudo apt-get install -y libjxl-tools libavif-bin libheif-examples libheif1 libopenjp2-7 librsvg2-bin potrace ghostscript libavif-gdk-pixbuf heif-gdk-pixbuf
```
